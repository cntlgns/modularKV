"""Focused sweep: KVLink0 8B finetune (step-4000) eval.

Same axes as sweep_8b_step2000.py, just the step-4000 checkpoint.

Job count: 1 model * 2 presets * 4 benches * 2 modes = 16
Output: result/ft_vs_pt_8b/  (shared with step-2000 — different model_slug
keeps stems disjoint).
"""
import os
import sys
...
MODELS = [
    f"{PROJECT_DIR}/model_cache/Llama-3.1-8B-Instruct-ft-step4000",
]
    ...
    for model in MODELS:
        for bench in BENCHMARKS:
            if bench == "wiki" and wiki_missing:
                print(f"  [skip] wiki/{model} (data missing)")
                continue
            for mode in MODES:
                positions = NQ_POS_SWEEP if (mode == "all" and bench == "nq") else [0]
                for pos in positions:
                    for preset in PROMPT_PRESETS:
                        ...
                        if done and not force:
                            print(f"  [skip] {stem} (exists; --force to rerun)")
                            continue
                        combos.append(
